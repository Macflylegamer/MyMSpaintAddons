    ...
    local function updateLabel(groupbox, targetText, newText, options)
        ...
        for _, child in ipairs(searchContainer:GetChildren()) do
            ...
            elseif options.delete then
                -- Handle the nested structure for the specific button deletion
                local nestedLabel = child:FindFirstChildOfClass('Frame') and child:FindFirstChildOfClass('Frame'):FindFirstChildWhichIsA('TextLabel')
                if nestedLabel and nestedLabel.Text == targetText then
                    child:Destroy()  -- Delete the frame containing the specific label
                    print("Deleted label frame containing:", targetText)
                    break
                end
            end
        end
